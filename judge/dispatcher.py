...
class SPJCompiler(DispatcherBase):
    def __init__(self, spj_code, spj_version, spj_language):
        ...
        spj_compile_config = list(
            filter(lambda config: spj_language == config["name"],
                   SysOptions.spj_languages))[0]["spj"]["compile"]
        self.data = {
            "src": spj_code,
    ...
    def compile_spj(self):
        with ChooseJudgeServer() as server:
            ...
            result = self._request(urljoin(server.service_url, "compile_spj"),
                                   data=self.data)
            if not result:
...
class JudgeDispatcher(DispatcherBase):
            ...
            try:
                for i in range(len(resp_data)):
                    if resp_data[i]["result"] == JudgeStatus.ACCEPTED:
                        resp_data[i]["score"] = self.problem.test_case_score[
                            i]["score"]
    ...
    def judge(self):
        ...
        data = {
            ...
            "src": code,
            "max_cpu_time": self.problem.time_limit * time_limit_mutil,
    ...
    def update_problem_status_rejudge(self):
        ...
        with transaction.atomic():
            ...
            profile = User.objects.select_for_update().get(
                id=self.submission.user_id).userprofile
            if problem.rule_type == ProblemRuleType.ACM:
                acm_problems_status = profile.acm_problems_status.get(
                    "problems", {})
                if acm_problems_status[problem_id][
                ...
                profile.save(
                    update_fields=["accepted_number", "acm_problems_status"])

            else:
                oi_problems_status = profile.oi_problems_status.get(
                    "problems", {})
                score = self.submission.statistic_info["score"]
                if oi_problems_status[problem_id][
                        "status"] != JudgeStatus.ACCEPTED:
                    # minus last time score, add this tim score
                    profile.add_score(
                        this_time_score=score,
                        last_time_score=oi_problems_status[problem_id]
                        ["score"])
                ...
                profile.save(
                    update_fields=["accepted_number", "oi_problems_status"])
    ...
    def update_problem_status(self):
        ...
        with transaction.atomic():
            ...
            if problem.rule_type == ProblemRuleType.ACM:
                ...
                elif acm_problems_status[problem_id][
                        "status"] != JudgeStatus.ACCEPTED:
                    acm_problems_status[problem_id][
                        "status"] = self.submission.result
                    if self.submission.result == JudgeStatus.ACCEPTED:
                ...
                if problem_id not in oi_problems_status:
                    ...
                    if self.submission.result == JudgeStatus.ACCEPTED:
                        user_profile.accepted_number += 1
                elif oi_problems_status[problem_id][
                        "status"] != JudgeStatus.ACCEPTED:
    ...
    def update_contest_problem_status(self):
        with transaction.atomic():
            ...
            if self.contest.rule_type == ContestRuleType.ACM:
                ...
                if problem_id not in contest_problems_status:
                ...
                elif contest_problems_status[problem_id][
                        "status"] != JudgeStatus.ACCEPTED:
                    contest_problems_status[problem_id][
                        "status"] = self.submission.result
                else:
                    ...
                    return
                user_profile.acm_problems_status[
                    "contest_problems"] = contest_problems_status
    ...
    def update_contest_rank(self):
        ...
        try:
            ...
            try:
                model.objects.create(user_id=self.submission.user_id,
    ...
    def _update_acm_contest_rank(self, rank):
        ...
        if info:
            ...
            if self.submission.result == JudgeStatus.ACCEPTED:
                ...
                rank.total_time += info[
                    "ac_time"] + info["error_number"] * 20 * 60
